import 'package:feedback_loop/presentation/screens/home_screen.dart';
import 'package:feedback_loop/presentation/screens/new_idea_screen.dart';
import 'package:feedback_loop/presentation/screens/profile_screen.dart';
import 'package:feedback_loop/presentation/widgets/scaffold_nav_bar.dart';
import 'package:go_router/go_router.dart';

final GoRouter appRouter = GoRouter(
  initialLocation: '/',
  routes: [
    ShellRoute(
      builder: (context, state, screen) => ScaffoldNavBar(screen: screen),
      routes: [
        GoRoute(
          path: '/',
          builder: (context, state) => const HomeScreen(),
        ),
        GoRoute(
          path: '/new-idea',
          builder: (context, state) => const NewIdeaScreen(),
        ),
        GoRoute(
          path: '/profile',
          builder: (context, state) => const ProfileScreen(),
        ),
      ],
    ),
  ],
);